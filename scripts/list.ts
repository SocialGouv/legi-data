...
const main = async () => {
  const sourceLines: string[] = await Promise.all(INDEXED_CODES.filter(code => code.etat === "VIGUEUR")
    .filter(code => fs.existsSync(path.join(__dirname, `../data/${code.id}.json`)))
    .map(async code => `${clean(code.id)} | ${clean(code.titre)} | ${clean(await getCodeLastUpdate(code.id))}`));
